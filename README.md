...
->  상인회와 협조를 통해 정찰제, 자체 답사, 사용자 참여하여 **품목 적정가 산정** 사용자 참여 오픈소스 생태계
...
- **평균가 조회:** 상인들이 제공한 가격 데이터와 직접 수집한 데이터를 바탕으로, 메인 화면을 통해 특정 시장 조회 시 시장별/품목별 평균가 정보를 조회하고 비교할 수 있는 기능을 제공 -> 직접 검색과 터치 방식으로 기능 이원화
...
- **데이터 수집:** 전통시장 시설 정보(주차장, 화장실 등)를 공공데이터 Open API와 데이터 크롤링, 사용자 생성 데이터를 통해 주기적으로 수집
...
- **데이터 수집 워크패키지**
    ...
    - **세부 기능:**
        - **API 연동:** 전통시장 기본 정보(위치, 시설 등) Open API를 주기적으로 호출하여 데이터 수집
        - **웹 크롤링:** API로 제공되지 않는 특정 시장의 가격 정보나 이벤트 정보를 얻기 위해, 서드파티 서비스(Apify, SerpApi, Outscraper)나 웹 크롤러(Python, Selenium/BeautifulSoup)를 이용해 대상 웹사이트의 데이터 수집
        - **사용자 생성 데이터 수집:** 사용자가 앱을 통해 직접 입력하는 리뷰, 사진, 가격 정보를 오픈 소스로 수집
- **데이터 전처리 워크패키지**
    - **역할:** 수집된 다양한 형태의 Raw 데이터를 정제하고 결합하여 서비스에 즉시 사용 가능한 형태로 가공
    - **세부 기능:**
        - **데이터 정제(Cleansing):** 수집된 데이터의 결측치, 이상치, 중복 값을 식별하고 처리
...
- **데이터 흐름**
    - **원천 데이터 → 전처리 → 데이터베이스 → 서버 → 클라이언트**
        1. **[원천 데이터]** 직접 수집, 오픈 API, 웹사이트 크롤링, 사용자 입력을 통해 정형/반정형/비정형 데이터를 수집
        2. **[전처리]** 수집된 데이터를 Pandas 등을 활용해 정제 및 통합하여 분석 가능한 형태로 가공
        3. **[데이터베이스]** 가공된 데이터를 PostgreSQL, MySQL 등 구조화된 데이터베이스에 저장
...
|  | Google Maps JS API / Google Places API / Naver Maps API | GPS 기반 정보 제공 및 길찾기 기능 구현 |
...
| **데이터베이스** | MySQL | 데이터의 안정적 저장 및 효율적인 위치 기반 쿼리 처리 |
...
- **문제 2: 저품질 및 악성 리뷰/가격 정보**
    ...
    - **해결 방안:**
        - **리뷰 모니터링:** 부적절한 단어를 감지하는 필터를 적용하고, 사용자 신고(flagging) 기능을 도입하여 커뮤니티가 자정작용을 할 수 있도록 유도
